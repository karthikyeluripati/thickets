"""Validates the completed 6-cell vision-encoder localization sweep (vision_early /
vision_middle / vision_late x relative-L2 radii {0.04, 0.07}, N=100 each) BEFORE any
aggregation. Reads only existing thicket_metrics.json / run_metadata.json files under
results/ -- runs no inference, perturbs no weights, modifies no scientific/experiment code.

Hard-checks, per cell and across all 6 cells: exactly 6 cells present; N=100; base_score
== 0.5600 (within float tolerance) for every cell; same model/task/dataset/prompt-scorer
identity and same 100 candidate seeds across all 6; restoration_mode=fixed_base;
perturbation_scale_mode=relative_l2; requested_relative_l2 correct per cell; 100 candidate
records with expert+tie+regression==100; no NaNs; the run's recorded git commit contains the
full engine-level encoder-cache-reset fix (0706a03) -- all three scopes are visual-affecting,
so this applies to every cell here, not just a subset.

On ANY failure, prints the exact cell + reason and exits 1. Never repairs or skips a bad
cell -- the caller (build_vision_localization_summary.py) refuses to aggregate unless this
exits 0.

Usage:
    python analysis/validate_vision_localization_6.py [--results-dir results] [--repo-root .]
"""
...
# Same fix commit validate_coarse_thicket_28.py checks against -- the full (scheduler +
# worker) encoder-cache reset, superseding the worker-only reset confirmed insufficient on
# GPU (RuntimeError: Encoder cache miss). All three scopes here are visual-affecting.
...
def main(argv=None) -> int:
    ...
    if len(cells) == 6:
        ...
        errors.append(("<sweep>", f"expected 6 loadable cells, got {len(cells)} -- see missing/malformed-file errors above"))

    print(f"=== Vision-localization sweep validation: {len(cells)}/6 cells loaded ===\n")
    if errors:
